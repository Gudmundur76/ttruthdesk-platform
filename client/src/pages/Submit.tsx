...
export default function Submit() {
  ...
  const [pmFetched, setPmFetched] = useState<{
    title: string;
    text: string;
    citation: string;
  } | null>(null);
    ...
    reader.onload = async ev => {
      ...
      if (file.type === "text/plain") {
        try {
          ...
          setFileText(
            `[File: ${file.name} — text will be extracted server-side]`
          );
        }
      } else {
        setFileText(
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="container py-24 text-center">
          <h2 className="text-2xl font-bold text-slate-900 mb-3">
            Sign in to submit documents
          </h2>
          <p className="text-slate-500 mb-6">
            Create a free account to start auditing molecular claims.
          </p>
          <Button asChild>
  ...
  const isSubmitting =
    submitText.isPending || submitFile.isPending || uploading;
      ...
      <div className="container py-12 max-w-2xl">
        <div className="mb-8">
          ...
          <p className="text-slate-500 text-sm">
            Upload or paste your document. Claims will be verified against the
            selected source domain using deterministic rules.
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-border p-6 shadow-sm">
          ...
          <div className="mb-6">
            ...
            {selectedDomain && (
              <p className="mt-1.5 text-xs text-muted-foreground">
                <span className="font-medium">Verifies:</span>{" "}
          ...
          <Tabs defaultValue="pubmed">
            <TabsList className="mb-5 w-full">
              <TabsTrigger value="pubmed" className="flex-1">
                PubMed / DOI
              </TabsTrigger>
              ...
              <form onSubmit={handlePubmedFetch} className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-1.5">
                    Fetches the abstract and methods section from PubMed or
                    Europe PMC. Open-access papers only.
                  </p>
                </div>
              </form>
                ...
                <form onSubmit={handlePubmedSubmit} className="mt-5 space-y-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={e =>
                        setPmFetched({ ...pmFetched, title: e.target.value })
                      }
                      className="mt-1.5"
                    />
                  </div>
                  <div>
                    <Label className="text-sm font-medium text-slate-700">
                      Fetched text (editable)
                    </Label>
                    <Textarea
                      value={pmFetched.text}
                      onChange={e =>
                        setPmFetched({ ...pmFetched, text: e.target.value })
                      }
                      rows={10}
        ...
        <div className="mt-5 rounded-lg border border-blue-100 bg-blue-50 p-4">
          ...
          <ol className="text-xs text-blue-600 space-y-1 list-decimal list-inside">
            ...
            <li>
              Each claim is checked against the selected verification domain
              using deterministic rules
            </li>
            <li>
              A scoped verdict is assigned with rationale, source coverage, and
              provenance stamp
            </li>
