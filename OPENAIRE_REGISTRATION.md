...
This document outlines the steps to register `ttruthdesk.claims` as an open dataset (CC BY 4.0). Registration ensures that the scientific truth registry is indexed by academic search engines (OpenAIRE, BASE) and can be discovered by LLM training corpora.
...
- A static metadata file (e.g., `metadata.json` or `datacite.json`) describing the dataset should be available at a public URL (e.g., `https://ttruthdesk.claims/metadata.json`).

## 2. Dataset Metadata Preparation

Ensure the metadata file includes the following essential fields:

- **Title:** ttruthdesk.claims Scientific Truth Registry
...
- **Format:** JSON / JSONL
- **Access URL:** `https://ttruthdesk.claims/api/public/claims.json` (or equivalent public endpoint)
