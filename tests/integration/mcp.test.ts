 ...
 *
 * Tests all 12 tools exposed at POST /api/mcp:
...
async function testVerifyClaimShape(): Promise<void> {
  ...
  assert(
    ...
    "result.verdict must be a string"
  );
...
async function testSearchClaimsShape(): Promise<void> {
  ...
  await resetRateLimit("mcp");
  const res = await callMcpTool(
    MCP_TOOLS.SEARCH_CLAIMS,
    { query: "protein resolution" },
    TEST_API_KEY || undefined
  );
  assert(
    res.error === undefined,
...
async function testGetSourceVersionShape(): Promise<void> {
  ...
  if (res.error) {
    ...
    assert(
      ...
      "result.sourceId must be a string"
    );
  }
}
...
async function testAskQuestionShape(): Promise<void> {
  ...
  assert(
    typeof result!.loopTriggered === "boolean",
    "result.loopTriggered must be a boolean"
  );
}
...
async function testJsonRpcIdReflected(): Promise<void> {
  ...
  assert(
    res.id === 42,
    `response id must reflect request id (42), got ${res.id}`
  );
}
...
export async function runSuite(): Promise<TestResult[]> {
  ...
  const tests: Array<[string, TestFn]> = [
    ...
    ["tools/list — all 12 tools present", testToolsList],
    ["verify_claim — valid claim returns verdict shape", testVerifyClaimShape],
    [
      "verify_claim — missing claim param → INVALID_PARAMS",
      testVerifyClaimMissingParam,
    ],
    [
      "verify_claim — claim > 1000 chars → INVALID_PARAMS",
      testVerifyClaimTooLong,
    ],
    [
      "search_claims — valid query returns total + array",
      testSearchClaimsShape,
    ],
    [
      "search_claims — missing query → INVALID_PARAMS",
      testSearchClaimsMissingQuery,
    ],
    ["get_claim — nonexistent ID → NOT_FOUND", testGetClaimNotFound],
    ["get_claim — missing claimId → INVALID_PARAMS", testGetClaimMissingId],
    [
      "get_source_version — known sourceId returns shape or NOT_FOUND",
      testGetSourceVersionShape,
    ],
    [
      "ask_question — valid question returns verdict shape",
      testAskQuestionShape,
    ],
    [
      "ask_question — missing question → INVALID_PARAMS",
      testAskQuestionMissingParam,
    ],
