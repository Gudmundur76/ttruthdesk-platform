...
export const documents = mysqlTable(
  "documents",
  {
    ...
    verticalDomain: varchar("verticalDomain", { length: 64 })
      .default("structural_biology")
      .notNull(),
    // Two-pass corpus quality tracking
    llmProvider: varchar("llmProvider", { length: 64 })
      .default("manus_builtin")
      .notNull(),
    qualityTier: mysqlEnum("qualityTier", ["draft", "verified"])
      .default("draft")
      .notNull(),
...
export const claims = mysqlTable(
  "claims",
  {
    ...
    // Phase 103: Composite Truth Signal — Stage 7 output
    // Combines upstream verdict + provenance score + chain distortion score into a single authoritative signal.
    compositeTruthScore: float("compositeTruthScore"), // 0.0–1.0; null = not yet computed
    compositeTruthLabel: mysqlEnum("compositeTruthLabel", [
      "verified_faithful", // Supported upstream + low chain distortion (< 0.25)
      "verified_distorted", // Supported upstream + high chain distortion (>= 0.25)
      "contradicted", // Contradicted upstream, low downstream distortion
      "contradicted_amplified", // Contradicted upstream + high downstream distortion (claim is wrong AND being spread)
      "partially_supported", // Partially Supported upstream, any chain state
      "contested", // Ambiguous upstream OR conflicting chain signals
...
// ─── Monitoring Feed ──────────────────────────────────────────────────────────
export const monitoringFeed = mysqlTable(
  "monitoring_feed",
  {
    id: int("id").autoincrement().primaryKey(),
    documentId: int("documentId").notNull(),
...
export const autoIngestedPapers = mysqlTable(
  "auto_ingested_papers",
  {
    ...
    verticalDomain: varchar("verticalDomain", { length: 64 })
      .default("structural_biology")
      .notNull(),
    ingestSource: mysqlEnum("ingestSource", ["pubmed", "biorxiv", "pdb_linked"])
      .default("pubmed")
      .notNull(),
  ...
  t => ({
    ...
    verticalIdx: index("auto_ingested_papers_vertical_idx").on(
      t.verticalDomain
    ),
  })
);
...
export const graphEntities = mysqlTable(
  "graph_entities",
  {
    id: int("id").autoincrement().primaryKey(),
  ...
  t => ({
    entityTypeNameIdx: index("entity_type_name_idx").on(
      t.entityType,
      t.canonicalName
    ),
    entityCanonicalUnique: uniqueIndex("entity_canonical_unique").on(
      t.entityType,
      t.canonicalName
    ),
  })
...
export const graphRelations = mysqlTable(
  "graph_relations",
  {
    id: int("id").autoincrement().primaryKey(),
...
export const userSubscriptions = mysqlTable(
  "user_subscriptions",
  {
    ...
    status: mysqlEnum("status", ["pending", "active", "cancelled", "refunded"])
      .default("pending")
      .notNull(),
...
export const predictionFeatures = mysqlTable(
  "prediction_features",
  {
    id: int("id").autoincrement().primaryKey(),
  ...
  t => ({
    entityFeatureIdx: index("pf_entity_feature_idx").on(
      t.entityId,
      t.featureType
    ),
...
export const predictionModels = mysqlTable(
  "prediction_models",
  {
    id: int("id").autoincrement().primaryKey(),
    ...
    validationResult: mysqlEnum("validationResult", [
      "correct",
      "incorrect",
      "pending",
    ]).default("pending"),
...
export const webhookAlerts = mysqlTable(
  "webhook_alerts",
  {
    id: int("id").autoincrement().primaryKey(),
    userId: int("userId").notNull(),
...
export const coordTasks = mysqlTable(
  "coord_tasks",
  {
    id: int("id").autoincrement().primaryKey(),
...
export const coordQueue = mysqlTable(
  "coord_queue",
  {
    id: int("id").autoincrement().primaryKey(),
...
export const coordContext = mysqlTable(
  "coord_context",
  {
    id: int("id").autoincrement().primaryKey(),
...
export const verticalAlerts = mysqlTable(
  "vertical_alerts",
  {
    id: int("id").autoincrement().primaryKey(),
    userId: int("userId").notNull(),
    ...
    notifyContradictions: boolean("notifyContradictions")
      .notNull()
      .default(true),
    ...
    frequency: mysqlEnum("frequency", ["instant", "daily", "weekly"])
      .notNull()
      .default("daily"),
    active: boolean("active").notNull().default(true),
  ...
  t => ({
    userVerticalUnique: uniqueIndex("va_user_vertical_unique").on(
      t.userId,
      t.verticalDomain
    ),
...
export const notificationLog = mysqlTable(
  "notification_log",
  {
    ...
    channel: mysqlEnum("channel", ["manus", "webhook"])
      .notNull()
      .default("manus"),
    /** Delivery status */
    status: mysqlEnum("status", ["sent", "failed", "skipped"])
      .notNull()
      .default("sent"),
    errorMsg: text("errorMsg"),
...
export const webhookDeliveryLog = mysqlTable(
...
export const claimProvenanceEvents = mysqlTable(
...
export type InsertClaimProvenanceEvent =
  typeof claimProvenanceEvents.$inferInsert;

// ─── Entity Co-occurrence ─────────────────────────────────────────────────────
// Tracks how often two entities appear together in the same document.
// Used to power the co-occurrence graph UI.
export const entityCooccurrences = mysqlTable(
  ...
  t => ({
    entityPairDocIdx: uniqueIndex("cooc_entity_pair_doc_idx").on(
      t.entityAId,
      t.entityBId,
      t.documentId
    ),
...
export const confidenceHistory = mysqlTable(
  "confidence_history",
  {
    id: int("id").autoincrement().primaryKey(),
    claimId: int("claimId").notNull(),
...
export const wikiPages = mysqlTable(
  "wiki_pages",
  {
    ...
    outboundLinks: json("outboundLinks")
      .$type<string[]>()
      .notNull()
      .default([]),
...
export const wikiLog = mysqlTable(
  "wiki_log",
  {
    id: int("id").autoincrement().primaryKey(),
...
export const metaAgentChecks = mysqlTable(
  "meta_agent_checks",
  {
    id: int("id").autoincrement().primaryKey(),
 ...
 */
export const predictionCalibration = mysqlTable(
...
export type InsertPredictionCalibration =
  typeof predictionCalibration.$inferInsert;
 ...
 */
export const overrideAuditLog = mysqlTable(
  "override_audit_log",
  {
    id: int("id").autoincrement().primaryKey(),
    claimId: int("claimId").notNull(),
 ...
 */
export const llmProviderQuality = mysqlTable(
  "llm_provider_quality",
  {
    ...
    allowedForHighStakes: boolean("allowedForHighStakes")
      .notNull()
      .default(true),
 ...
 */
export const knowledgeGaps = mysqlTable(
  "knowledge_gaps",
  {
    id: int("id").autoincrement().primaryKey(),
    ...
    detectionSource: varchar("detectionSource", { length: 64 })
      .notNull()
      .default("frontier_scan"),
    /**
 ...
 */
export const frontierLog = mysqlTable(
  "frontier_log",
  {
    id: int("id").autoincrement().primaryKey(),
    /**
 ...
 */
export const selfPromptLog = mysqlTable(
  "self_prompt_log",
  {
    id: int("id").autoincrement().primaryKey(),
 ...
 */
export const generatedClaims = mysqlTable(
  "generated_claims",
  {
    id: int("id").autoincrement().primaryKey(),
    claimText: text("claimText").notNull(),
    ...
    parentVerifications: json("parentVerifications")
      .$type<number[]>()
      .notNull(),
 ...
 */
export const dreamSessions = mysqlTable(
  "dream_sessions",
  {
    id: int("id").autoincrement().primaryKey(),
    startedAt: timestamp("startedAt").defaultNow().notNull(),
    ...
    confidenceRecalibrations: int("confidenceRecalibrations")
      .notNull()
      .default(0),
    simulatedScenarios: int("simulatedScenarios").notNull().default(0),
    patternLog: json("patternLog").$type<
      Array<{
        type: string;
        description: string;
    ...
    >(),
    simulationLog: json("simulationLog").$type<
      Array<{
        scenario: string;
    ...
    >(),
    recalibrationLog: json("recalibrationLog").$type<
      Array<{
        claimId: number;
 ...
 */
export const eventQueue = mysqlTable(
  "event_queue",
  {
    id: int("id").autoincrement().primaryKey(),
 ...
 */
export const loopRun = mysqlTable(
  "loop_run",
  {
    id: int("id").autoincrement().primaryKey(),
 ...
 */
export const verticalConfigs = mysqlTable(
  "vertical_configs",
  {
    ...
    sourceWhitelist: json("sourceWhitelist")
      .$type<string[]>()
      .notNull()
      .default([]),
 ...
 */
export const cronRunLog = mysqlTable(
  "cron_run_log",
  {
    id: int("id").primaryKey().autoincrement(),
 ...
 */
export const micronDeployments = mysqlTable(
  "micron_deployments",
  {
    id: int("id").primaryKey().autoincrement(),
    ...
    deployTarget: mysqlEnum("deployTarget", [
      "vercel",
      "netlify",
      "docker",
      "ipfs",
...
export const discoveryRuns = mysqlTable(
  "discovery_runs",
  {
    ...
    registeredSources: json("registeredSources")
      .$type<string[]>()
      .notNull()
      .default([]),
    ...
    runLog: json("runLog")
      .$type<Array<{ phase: string; message: string; ts: number }>>()
      .notNull()
      .default([]),
    startedAt: timestamp("startedAt").defaultNow().notNull(),
    completedAt: timestamp("completedAt"),
 ...
 */
export const sourceRegistryEntries = mysqlTable(
...
export type InsertSourceRegistryEntry =
  typeof sourceRegistryEntries.$inferInsert;

// ─── Saved Research ───────────────────────────────────────────────────────────
export const savedResearch = mysqlTable(
  "saved_research",
  {
    id: int("id").autoincrement().primaryKey(),
    userId: int("userId").notNull(),
...
export const publicSubmissions = mysqlTable(
  "public_submissions",
  {
    id: int("id").autoincrement().primaryKey(),
    claimText: text("claimText").notNull(),
...
export const siaGenerations = mysqlTable(
  "sia_generations",
  {
    id: int("id").autoincrement().primaryKey(),
...
export const siaImprovementProposals = mysqlTable(
  "sia_improvement_proposals",
  {
    id: int("id").autoincrement().primaryKey(),
    runId: varchar("runId", { length: 64 }).notNull(),
    generation: int("generation").notNull(),
...
export type SiaImprovementProposal =
  typeof siaImprovementProposals.$inferSelect;
export type InsertSiaImprovementProposal =
  typeof siaImprovementProposals.$inferInsert;

// ─── Citation Chain Analysis ───────────────────────────────────────────────────
export const citationEdges = mysqlTable(
  "citation_edges",
  {
    id: int("id").autoincrement().primaryKey(),
...
// ─── Graph Claim Edges ────────────────────────────────────────────────────────
// Directed edges between claims in the knowledge graph.
// relationType: semantic_similar | cites | contradicts | supports | refines
// weight: 0.0–1.0 (cosine similarity for semantic_similar; 1.0 for explicit relations)
export const graphClaimEdges = mysqlTable(
  "graph_claim_edges",
  {
    id: int("id").autoincrement().primaryKey(),
    sourceClaimId: int("sourceClaimId").notNull(),
    targetClaimId: int("targetClaimId").notNull(),
    relationType: mysqlEnum("relationType", [
      "semantic_similar",
      "cites",
      "contradicts",
      ...
      .notNull(),
    weight: float("weight").default(1.0).notNull(), // cosine sim or explicit strength
    createdAt: timestamp("createdAt").defaultNow().notNull(),
  },
  t => ({
    sourceIdx: index("gce_source_idx").on(t.sourceClaimId),
    targetIdx: index("gce_target_idx").on(t.targetClaimId),
    relationIdx: index("gce_relation_idx").on(t.relationType),
    weightIdx: index("gce_weight_idx").on(t.weight),
    // Unique constraint to prevent duplicate edges of the same type
    uniqueEdge: uniqueIndex("gce_unique_edge").on(
      t.sourceClaimId,
      t.targetClaimId,
      t.relationType
    ),
...
export type GraphClaimEdge = typeof graphClaimEdges.$inferSelect;
export type InsertGraphClaimEdge = typeof graphClaimEdges.$inferInsert;

// ─── Contradiction Alerts ─────────────────────────────────────────────────────
// Phase 107: Contradiction Detection Engine
//
// Stores detected contradictions between claim pairs connected by semantic_similar
// edges in the knowledge graph. A contradiction is flagged when one claim has a
// "positive" composite truth label (verified_faithful, partially_supported) and
// its graph neighbour has a "negative" label (contradicted, contradicted_amplified).
//
// The contradiction_scan heartbeat (weekly) refreshes this table idempotently.
// The unique index on (claimAId, claimBId) prevents duplicate rows; the engine
// always stores pairs with claimAId < claimBId for canonical ordering.
export const contradictionAlerts = mysqlTable(
  "contradiction_alerts",
  {
    id: int("id").autoincrement().primaryKey(),
    claimAId: int("claimAId").notNull(),
    claimBId: int("claimBId").notNull(),
    claimAVerdict: varchar("claimAVerdict", { length: 64 }),
    claimBVerdict: varchar("claimBVerdict", { length: 64 }),
    claimALabel: varchar("claimALabel", { length: 64 }),
    claimBLabel: varchar("claimBLabel", { length: 64 }),
    claimAScore: float("claimAScore"),
    claimBScore: float("claimBScore"),
    edgeWeight: float("edgeWeight").default(0.5).notNull(),
    severity: mysqlEnum("severity", ["high", "medium", "low"])
      .default("medium")
      .notNull(),
    status: mysqlEnum("status", ["open", "reviewed", "resolved", "dismissed"])
      .default("open")
      .notNull(),
  ...
  t => ({
    claimAIdx: index("ca_claim_a_idx").on(t.claimAId),
    claimBIdx: index("ca_claim_b_idx").on(t.claimBId),
    severityIdx: index("ca_severity_idx").on(t.severity),
    statusIdx: index("ca_status_idx").on(t.status),
    detectedAtIdx: index("ca_detected_at_idx").on(t.detectedAt),
    uniquePair: uniqueIndex("ca_unique_pair").on(t.claimAId, t.claimBId),
  })
);
export type ContradictionAlert = typeof contradictionAlerts.$inferSelect;
export type InsertContradictionAlert = typeof contradictionAlerts.$inferInsert;

// ─── Claim Score History ──────────────────────────────────────────────────────
// One row per claim per re-evaluation run where the score changed (or on first
// scoring). Provides the data for the Claim Confidence Timeline sparkline.
//
// Written by reEvaluationEngine.reScoreClaim() whenever a claim score is
// updated. Also written by the analysis pipeline on first composite scoring.
//
// The unique index on (claimId, snapshotAt) prevents duplicate rows if the
// engine is accidentally called twice in the same second.
export const claimScoreHistory = mysqlTable(
  "claim_score_history",
  {
    id: int("id").autoincrement().primaryKey(),
    claimId: int("claimId").notNull(),
    compositeTruthScore: float("compositeTruthScore").notNull(),
    compositeTruthLabel: varchar("compositeTruthLabel", { length: 64 }),
