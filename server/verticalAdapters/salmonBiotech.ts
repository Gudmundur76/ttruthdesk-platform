 ...
 *
 * Evidence sources:
 *   - PubChem (compound identity and bioactivity)
 *   - RCSB PDB (structural data for salmon-derived proteins)
 *   - FAO FishBase (species and aquaculture data)
 *
 * STATUS: Scaffold — evidence lookup returns a structured stub.
 * Full PubChem + FishBase integration is the next implementation step.
 *
...
const KNOWN_COMPOUNDS: Record<string, { cid: number; name: string }> = {
  astaxanthin: { cid: 5281224, name: "Astaxanthin" },
  "omega-3": { cid: 5280934, name: "Eicosapentaenoic acid (EPA)" },
  epa: { cid: 5280934, name: "Eicosapentaenoic acid (EPA)" },
  dha: { cid: 445580, name: "Docosahexaenoic acid (DHA)" },
  collagen: { cid: 73995, name: "Collagen (type I)" },
  "fish oil": { cid: 5280934, name: "Fish oil (EPA reference)" },
  "marine peptide": { cid: 0, name: "Marine bioactive peptide (generic)" },
};

async function lookupPubChem(compoundName: string): Promise<EvidenceResult> {
  const normalised = compoundName.toLowerCase().trim();
  const known = KNOWN_COMPOUNDS[normalised];

  if (!known || known.cid === 0) {
    // Stub: full PubChem text search not yet implemented
    return {
      found: false,
      sourceId: null,
      sourceUrl: null,
      ...
      confidenceScore: 0.4,
      confidenceFlags: [
        "PubChem full-text search not yet implemented for this vertical",
        `Compound '${compoundName}' not in known compound registry`,
  ...
  try {
    const res = await fetch(
      `https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/${known.cid}/JSON`
    );
    if (!res.ok) {
      return {
        found: false,
        ...
        evidenceRaw: null,
        confidenceScore: 0.5,
        confidenceFlags: [`PubChem lookup failed for CID ${known.cid}`],
      };
    }
    ...
    return {
      ...
      evidenceRaw: null,
      confidenceScore: 0.3,
      confidenceFlags: [`PubChem request failed: ${String(err)}`],
    };
  }
}
...
const salmonBiotechAdapter: VerticalAdapter = {
  ...
  description:
    ...
    "and marine bioactive peptides. Evidence sourced from PubChem, RCSB PDB, and " +
    "FAO FishBase. Developed for Icelandic land-based aquaculture (Laxey, Vestmannaeyjar).",
...
`,
  ...
  async lookupEvidence(claim) {
    // Extract compound name from claim text
