...
export async function fetchPendingDreamItems(limit = 10): Promise<DreamQueueItem[]> {
...
export async function rejectDreamItem(id: number, reason?: string): Promise<void> {
  const db = await getDb();
  if (!db) return;
  await db
    .update(dreamStagingQueue)
    .set({ status: "rejected", reviewedAt: Date.now(), reviewNote: reason ?? null })
    .where(eq(dreamStagingQueue.id, id));
}
...
export async function rejectStaleDreamItems(): Promise<number> {
  ...
  const stale = await db
    ...
    .from(dreamStagingQueue)
    .where(and(eq(dreamStagingQueue.status, "pending"), lt(dreamStagingQueue.createdAt, cutoff)));
...
export async function consumeDreamQueue(options: DreamQueueConsumerOptions = {}): Promise<DreamConsumerResult> {
