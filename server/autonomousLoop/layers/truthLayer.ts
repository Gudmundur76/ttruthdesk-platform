...
export async function runTruthLayer(event: LoopEvent): Promise<TruthLayerResult> {
  ...
  await emitLayerStart("L1_TRUTH", corrId, { eventQueueId: event.id });
  ...
  try {
    ...
    await emitLayerError("L1_TRUTH", corrId, "LAYER_ERROR", {
      eventQueueId: event.id,
      durationMs: Date.now() - startMs,
    });
    log.error("[TruthLayer] Unhandled error:", errData(err as Error));
    return { actions, verdicts };
  }

  await emitLayerEnd("L1_TRUTH", corrId, startMs, { eventQueueId: event.id });
  return { actions, verdicts };
}
